...
class ProcessedPage:
    ...
    def __daraframe_to_list_of_dict(self, cells_df: pd.DataFrame) -> list[dict]:
        ...
        Output:
        ------------
        list of data frame rows in dict format:
    ...
    def __generate_rows(self, cells: list[dict]) -> pd.DataFrame:
        ...
        Input:
        -------------
        cells: list of cells, each cell is individual dictionary 
        with keys: "bbox", and "text"

        Output:
        -------------
        returns: A dataframe, each row in df will have row bbox and cells
    ...
    def get_word_df_within_bbox(self, bbox: list) -> pd.DataFrame:
        ...
        Input:
        -------------
        bbox: list of 4 coordinates of bbox

        Output:
        -------------
        returns: A dataframe, each row in df will have word bbox and text

        """
        df = self.get_word_df()
        mask = df.apply(lambda row: self.__iob([row.x0, row.y0, row.x1, row.y1], bbox) > 0.6, axis=1)
        df = df[mask]
    ...
    def get_line_df_within_bbox(self, bbox: list) -> pd.DataFrame:
        ...
        Output:
        -------------
        returns: A dataframe, each row in df will have line bbox 
        and and all the cells in that line, the dataframe will have
        two columns: "bbox" and "cells"

        """
        df = self.get_span_df_within_bbox(bbox)
        cells_in_dict_format = self.__daraframe_to_list_of_dict(df)
